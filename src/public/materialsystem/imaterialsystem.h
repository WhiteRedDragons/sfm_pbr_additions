...
struct FlashlightState_t
{
	...
	bool  m_bGlobalLight : 1;

	// simple projection
	bool	m_bSimpleProjection : 1;
	bool	m_bSimpleLight : 1;

	// Uberlight parameters
	bool m_bUberlight : 1;
	UberlightState_t m_uberlightState;

	// ficool2: anything beyond here is not correct
	// only used by lightshafts so we don't care for now
...
private:
	...
	// ficool2: unknown data
	float m_Unk[27];

	IMPLEMENT_OPERATOR_EQUAL( FlashlightState_t );
};

static_assert( sizeof( FlashlightState_t ) == 348 );
